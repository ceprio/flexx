...
.. UIExample:: 100
    ...
    class Example(e.Element):
        def init(self):
            ...
            return [e.Button(text='foo'), e.Button(text='bar')]
...
class Element(app.JsComponent):
    ...
    children = app.LocalProperty((), doc="""
        The child elements of this element. This property is not settable and
    ...
    def __render_resolve(self, vnode, node=None):
        ...
        vnode: this is the node to be instanciated in the DOM in place of 
               the anchor node or as a children. 
        ...
        if vnode.children is None:
            ...
            # Truncate children of anchor node
            ...
            for i2 in range(len(vnode.children)):
                ...
                subnode = None
                if i1 < len(node.childNodes): # if anchor node has children; match them with vsubnode
                ...
                new_subnode = self.__render_resolve(vsubnode, subnode) # recurse
                if subnode is None:
                    node.appendChild(new_subnode)
                elif subnode is not new_subnode: # replace anchor children
